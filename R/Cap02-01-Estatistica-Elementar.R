...
mean(dataset$age)  
mean(dataset$age, na.rm = TRUE) # -> na.rm -> remove valores missing 
sum(is.na(dataset$age)) # -> total de valores missing
...
# Para resolver problemas na instalação -> sudo -i R
...
skewness(dados)
hist(dados)
# os dados é uma distribuição normal
kurtosis(dados)
# a kurtosis é zero.
