...
# Carregando os dados
dados <- read.csv("dados/dataset.csv", header = TRUE, stringsAsFactors = FALSE)
dim(dados)
str(dados)
...
hist(dados$AGE)
hist(dados$AGE_DON)
hist(dados$PTIME) # numero de dias que a pessoa que recebeu o transplante viveu.
hist(dados$DAYSWAIT_CHRON) # tempo de espera
hist(dados$FINAL_MELD_SCORE) # pontuação de acordo com a severidade da doença
...
# Correlação entre as variáveis numéricas -> matrix correlation
